...
- `/rankings` — **Expert player rankings**: FantasyPros Expert Consensus redraft **overall** (filter by position — no kickers; **ALL** omits kickers), plus **Keeper: draft vs consensus** — last completed Sleeper startup/snake draft vs current redraft ECR for rostered players only (Human League rule: lose pick in drafted round; **undrafted = round 14** at **10 teams × 14 rounds**, wired in `src/config.js` as `leagueFormat`). Data: [DynastyProcess](https://github.com/dynastyprocess/data) ECR + id map (`db_fpecr_latest.csv`, `db_playerids.csv`) and Sleeper drafts/rosters. API caches ECR 1h; keeper merge runs in the browser.
...
- **Client:** `src/pages/Rankings.jsx` — **Redraft rankings** (Bye / SD / Best-Worst / Owned) vs **Keeper: draft vs consensus** (startup draft round, overall pick, **round cost** including undrafted = round from `leagueFormat.undraftedKeeperRound`, ECR, **Δ** = pick − ECR). Keeper mode walks `previous_league_id` chain until it finds a season whose primary snake draft has picks, loads those picks plus **rosters for that league**, and merges with ECR via Sleeper player ids. League constants live in `leagueFormat` in `src/config.js`. Kickers excluded from filters and from ALL. Scraped date shown with staleness hint after 14 days.
