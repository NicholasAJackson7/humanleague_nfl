...
- **Server:** `api/rankings.js` fetches all three files on first call, parses + joins in memory, and serves filtered slices keyed by `page_type`. Real FantasyPros pages (`redraft-overall`, `dynasty-rb`, etc.) come from the ECR file; the synthetic `keeper-values-1qb` and `keeper-values-2qb` page types come from `values.csv` and add per-player `value`, `age`, `draft_year`, and dynasty positional rank to the response. Cache is held in module scope for **1 hour**; on upstream failure the function serves the last-known-good response for up to **24 hours**. The HTTP response also sets `Cache-Control: public, s-maxage=3600, stale-while-revalidate=86400` so Vercel's edge cache amplifies the in-process cache across function instances.
- **Client:** `src/pages/Rankings.jsx` is a sortable table with format dropdown, position pill filter (overall and keeper-values views), and name/team search. The table swaps its trailing columns automatically — Bye / SD / Best-Worst / Owned for ECR pages, Age / Value / Dyn-Rank / Pos-Rank for keeper-values pages. The page header displays the upstream `scrape_date` and warns when it's more than 14 days old.
